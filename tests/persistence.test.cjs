...
const user={uid:'owner'},rootPath='users/owner';const root=(revision=0)=>({schemaVersion:5,revision,migrationV5:{status:'complete'}});
const catalog=[{id:'w_000001',english:'penetrate',meaning:'穿透',partOfSpeech:['verb'],lessonIds:['U6']},{id:'w_000002',english:'record',meaning:'紀錄',partOfSpeech:['noun'],lessonIds:['U8']}];
...
test('an empty account writes only a v5 completion marker, never copies catalog or downloads history',async()=>{
...
test('one field writes one sparse override; restoring equality deletes entire empty document',async()=>{
 const h=harness({[rootPath]:root()});const m=createModel(),before=m.exportState();m.updateWordOverride('w_000001',{meaning:''});
 await h.api.save(before,m.exportState(),user,{expectedRevision:0});
 assert.deepEqual(Object.keys(h.documents.get(`${rootPath}/wordOverrides/w_000001`)).sort(),['meaning','schemaVersion','updatedAt']);
 const next=m.exportState();m.clearWordOverrideField('w_000001','meaning');await h.api.save(next,m.exportState(),user,{expectedRevision:1});
...
test('legacy roots and source documents are retained and backed up before canonical replacements',async()=>{
 ...
 const loaded=await h.api.load(user);assert.equal(loaded.snapshot.userOverrides.w_000001.meaning,'latest');assert.deepEqual(loaded.snapshot.hiddenWordIds,['w_000001']);
...
test('migration failure during staging changes no canonical records and retry retains complete original input',async()=>{
 ...
 assert.notEqual(h.documents.get(rootPath).migrationV5.status,'complete');h.control.beforeCommit=null;
 const loaded=await h.api.load(user);assert.equal(loaded.snapshot.customWords.c.meaning,'original');
...
test('failure after a migration batch is durable resumes frozen plan and never exposes partial data',async()=>{
 ...
 await assert.rejects(h.api.load(user),error=>error.code==='cloud-partial-commit');assert.notEqual(h.documents.get(rootPath).migrationV5.status,'complete');
 const backupId=h.documents.get(rootPath).migrationV5.backupId;assert.equal(h.documents.get(`${rootPath}/migrationBackups/${backupId}`).cursor,1);
 ...
 assert.equal(h.documents.get(rootPath).migrationV5.backupId,backupId);assert.equal(loaded.revision,1);
...
test('lost acknowledgement of final migration commit does not repeat migration or resurrect edits',async()=>{
 ...
 h.control.afterCommit=changes=>{if(!failed&&changes.some(c=>c.path===rootPath&&c.value?.migrationV5?.status==='complete')){failed=true;throw new Error('lost ack');}};
...
test('cross-device revision mismatch rejects the whole write without overwriting another device',async()=>{
 const h=harness({[rootPath]:root()});const a=createModel(),b=createModel();a.updateWordOverride('w_000001',{meaning:'A'});b.updateWordOverride('w_000001',{meaning:'B'});
 ...
 assert.equal((await h.api.load(user)).snapshot.userOverrides.w_000001.meaning,'A');
});
...
test('accounts have isolated storage paths and active locks/future schemas fail closed',async()=>{
 const h=harness({[rootPath]:root(),'users/other':root()});const m=createModel();m.updateWordOverride('w_000001',{meaning:'owner only'});
 ...
 h.documents.set(rootPath,{schemaVersion:6});await assert.rejects(h.api.load(user),error=>error.code==='unsupported-schema-version');
});
